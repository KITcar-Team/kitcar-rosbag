...
if [ $# -eq 0 ]
then
	tput setaf 2; echo "Usage: sh play.sh ROSBAG_DIRECTORY [ROSBAG_PLAY_OPTIONS] [-n|--no-params]"
	echo "or"
	echo "Usage: sh play.sh ROSBAG_FILE [ROSBAG_PLAY_OPTIONS] [-n|--no-params]"; tput sgr0
	echo ""
	echo "Load parameters and play rosbag created by record.sh."
	echo ""
	echo "-n, --no-params		skip parameter loading"
	echo ""
	tput setaf 2; echo "ROSBAG_DIRECTORY: the folder containing the .bag file and the .yaml file created by record.sh"
	echo "ROSBAG_FILE: the .bag file. Use the ROSBAG_DIRECTORY if the rosbag was created by record.sh" ; tput sgr0
	echo ""
...
#the newline above is important
    ...
    # This is neccessary to handle legacy rosbags.
    # World Coordinate Frame was renamed to Odom
    # This Code block creates a link between Odom and World as Affine Identity
    # 1593471600 = Tuesday, June 30, 2020 1:00:00 AM GMT+02:00
    rosbag_creation_time=$(rosbag info $rosbagdirectory | grep start | grep -Eo "[0-9]{10}")
    if [ "$rosbag_creation_time" -lt 1593471600 ]
    then
        rosrun tf2_ros static_transform_publisher 0 0 0 0 0 0 odom world __name:=odom2world_legacy_rosbag_link &
    fi
...
then
	...
	then
		...
		then
			...
			echo "Use the ROSBAG_DIRECTORY option if the rosbag was created by record.sh"
			echo ""
			{
			read_params=true
			while true; do
		...
		echo ""
		tput setaf 2; echo "Usage: sh play.sh ROSBAG_DIRECTORY [ROSBAG_PLAY_OPTIONS]"
		echo "or"
		echo "Usage: sh play.sh ROSBAG_FILE [ROSBAG_PLAY_OPTIONS]"; tput sgr0
		echo ""
		tput setaf 2; echo "ROSBAG_DIRECTORY: the folder containing the .bag file and the .yaml file created by record.sh"
		echo "ROSBAG_FILE: the .bag file. Use the ROSBAG_DIRECTORY if the rosbag was created by record.sh" ; tput sgr0
	fi
